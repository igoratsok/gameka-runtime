	...
	for(std::list<GameObject*>::iterator it = spatialList->begin(); it != spatialList->end(); ++it) {
		...
		if(autoCollision && gameObject != this &&
			...
			if(gameObject->gameObjectInstance->gameObject->derrotaEncostandoInimigo) {
				...
				Rectangle *thisRect = this->sprite->getRectWithoutRot();
			...
			if(!bouncedOnce && this->gameObjectInstance->
				...
				Rectangle *gameObjectRect = gameObject->sprite->getRectWithoutRot();
...
void BouncingBall::bounce(Rectangle rect, int dt, Vector2 *objVelocity) {
	float tfirst, tlast;
	int lado;
	Rectangle movedRect = Rectangle(0, 0, 0, 0);
	Vector2 realVelocity = Vector2(velocity->x*dt, velocity->y*dt);

	playSpecialSound();

	Rectangle *objectRect = this->sprite->getRectWithoutRot();
	...
	if(objectRect->movingIntersectRect(realVelocity, rect, &tfirst, &tlast, &lado, &movedRect)) {
		...
		if(lado == Rectangle::SIDE_RIGHT || lado == Rectangle::SIDE_LEFT) {
