...
void Sprite::moveToScreenCenter() {
	GameData *gameData = GameData::getInstance();
	Rectangle *rect = getRectWithoutRot();
...
Rectangle* Sprite::getRect() {
	if(rotAngle == 0 && zoom == 1) {
		...
		rectRot = createRectRot(textureRotoZoomed);
		return new Rectangle(rectRot.x, rectRot.y, rectRot.w, rectRot.h);
	}

}

Rectangle* Sprite::getRectExact() {
	if(rotAngle == 0 && zoom == 1) {
			return new Rectangle(rectangle.x, rectangle.y, rectangle.w, rectangle.h);
	} else {
		SDL_Rect rectRot;
...
Rectangle* Sprite::getRectWithoutRot() {
...
void Sprite::drawRectNoScrolling(Rectangle rect, int r, int g, int b, int a, Screen *screen) {
