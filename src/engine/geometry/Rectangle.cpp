...
bool Rectangle::intersectsXAxisWith(Rectangle *rectangle) {
...
bool Rectangle::intersectsYAxisWith(Rectangle *rectangle) {
...
bool Rectangle::contains(Vector2 vector) {
	return (vector.x >= this->x && vector.x <= this->x + this->w &&
			vector.y >= this->y && vector.y <= this->y + this->h);
}

Vector2 Rectangle::getMin() {
	return Vector2(x, y);
}

Vector2 Rectangle::getMax() {
	return Vector2(x + w, y + h);
}

bool Rectangle::rayIntersection(Vector2 p, Vector2 f, float *tmin, Vector2 *q) {
	*tmin = 0.0f;
	float tmax = INFINITY;
	Vector2 d;
	d.x = f.x - p.x;
...
bool Rectangle::intersectMovingRectRect(Rectangle a, Rectangle b, Vector2 va, Vector2 vb,
...
bool Rectangle::sweepTestRectRect(Rectangle a, Rectangle b, Vector2 va, Vector2 vb, float *tfirst, float *tlast) {
	Vector2 v;
	v.x = vb.x - va.x;
	v.y = vb.y - va.y;
...
bool Rectangle::movingIntersectRect(Vector2 v, Rectangle rect, float *tfirst, float *tlast, int *lado, Rectangle *movedRectCol) {
	bool result = Rectangle::intersectMovingRectRect(*this, rect, v, Vector2(0, 0), tfirst, tlast);
	...
	Rectangle movedRectNoRound = *this;
